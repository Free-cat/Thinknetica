...
class Main
  ...
  def test_date
    ...
    @trains << ct1 = CargoTrain.new("cargo_train1", 'cargo_train1')
    @trains << ct2 = CargoTrain.new("cargo_train2", 'cargo_train2')
    @trains << ct3 = CargoTrain.new("cargo_train3", 'cargo_train3')
    @trains << pt1 = PassengerTrain.new("passenger_train1", 'passenger_train1')
    @trains << pt2 = PassengerTrain.new("passenger_train2", 'passenger_train2')
    @trains << pt3 = PassengerTrain.new("passenger_train3", 'passenger_train3')
...
#Добавлять вагоны к поезду
  def attach_wagon_to_train
    loop do
      ...
      wagon_company_name = show_message("Введите производителя поезда или ENTER для завершения")
      ...
      if train.type == "cargo"
        ...
        puts "Поезд с ID #{train.id} не существует!"
      end
    end
  end
#Отцеплять вагоны от поезда
  ...
  def show_train
    @trains.each_with_index do |train, index|
      puts "[#{index}] Поезд #{train.id} - количество вагон #{train.wagons_count.size} Производитель #{train.company_name}"
    end
  end
