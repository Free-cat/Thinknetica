  ...
  def create_trains
    loop do
      ...
      if train_type == 1
        ...
        puts "Пассажирский поезд c номером #{train.id} успешно создан"
        @trains << train
      elsif train_type == 2
        train = CargoTrain.new(train_id)
        puts "Товарный поезд c номером #{train.id} успешно создан"
        @trains << train
      elsif train_id.nil?
        return
      else
  ...
  def attach_wagon_to_train
    loop do
      ...
      if train.type == "cargo"
        ...
        train.add_wagon(wagon)
        puts "Вагон прицеплен к поезду #{train.id}"
      else
...
#Отцеплять вагоны от поезда
  ...
  def show_train
    @trains.each_with_index do |train, index|
      puts "[#{index}] Поезд #{train.id} - количество вагон #{train.wagons_count.size}"
    end
  end
